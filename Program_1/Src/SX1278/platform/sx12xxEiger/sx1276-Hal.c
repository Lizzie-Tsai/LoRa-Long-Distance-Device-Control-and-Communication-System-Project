...
#define NSS_PIN                                     GPIO_PIN_8     //ԭGPIO_Pin_15
...
#define RXE_HIGH()         		HAL_GPIO_WritePin(RXE_PORT,RXE_PIN,GPIO_PIN_SET)//; GPIO_SetBits(RXE_PORT,RXE_PIN)
#define RXE_LOW()          		HAL_GPIO_WritePin(RXE_PORT,RXE_PIN,GPIO_PIN_RESET)//GPIO_ResetBits(RXE_PORT,RXE_PIN)
...
#define TXE_HIGH()         		HAL_GPIO_WritePin(TXE_PORT,TXE_PIN,GPIO_PIN_SET)//GPIO_SetBits(TXE_PORT,TXE_PIN)
#define TXE_LOW()          		HAL_GPIO_WritePin(TXE_PORT,TXE_PIN,GPIO_PIN_RESET)//GPIO_ResetBits(TXE_PORT,TXE_PIN)
...
void SX1276WriteBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    //NSS = 0;
			...
			//if(MachType==MASTER)GPIO_WriteBit( NSS_IOPORT, NSS_PIN, Bit_RESET );
    ...
    //NSS = 1;
			...
			//if(MachType==MASTER)GPIO_WriteBit( NSS_IOPORT, NSS_PIN, Bit_SET );
			//else GPIO_WriteBit( GPIOA, GPIO_Pin_4, Bit_SET );
	//	GPIO_WriteBit( NSS_IOPORT, NSS_PIN, Bit_SET );
...
{
	//printf("IO:%d\r\n",HAL_GPIO_ReadPin( DIO0_IOPORT, DIO0_PIN ));
	if(MachType==MASTER){
