...
void LCD_Scan_Dir(uint8_t dir)
{
	...
	if(LCD_Code==ILI9341)//9341/6804/5310/5510,很特殊
	{
		dir=D2U_L2R;
		switch(dir)
		{
...
static void LCD_FSMCConfig(void)
{
   ...
   SRAM_Handler.Init.BurstAccessMode=FMC_BURST_ACCESS_MODE_DISABLE; 
	...
	SRAM_Handler.Init.PageSize = FMC_PAGE_SIZE_NONE;
	      ...
	      FMC_ReadWriteTim.AddressSetupTime=1;        //地址建立时间(ADDSET)为15个HCLK 1/216M*15=4.6ns*15
        FMC_ReadWriteTim.AddressHoldTime=0;
        FMC_ReadWriteTim.DataSetupTime=15;           //数据保存时间(DATAST)为255个HCLK        =4.6*255
        ...
        //FMC写时序控制寄存器
        FMC_WriteTim.AddressSetupTime=2;            //地址建立时间(ADDSET)为17个HCLK=82.5ns
        FMC_WriteTim.AddressHoldTime=0;
        FMC_WriteTim.DataSetupTime=5;               //数据保存时间(DATAST)为5.5ns*17个HCLK=82.5ns
...
//		printf("ss\r\n");
//		LCD_WriteReg(0x50, 0); //姘村钩GRAM璧峰浣嶇疆 
//		LCD_WriteReg(0x51, 240); //姘村钩GRAM缁堢偣浣嶇疆 
//		LCD_WriteReg(0x52, 0); //姘村钩GRAM璧峰浣嶇疆
//		LCD_WriteReg(0x53, 319); //鍨傜洿GRAM缁堢偣浣嶇疆
